...
// Project Image Component with gradient placeholders
const ProjectImage = ({ 
  project, 
  className 
}: { 
  ...
  const getIcon = (tech: string[]) => {
    if (tech.some(t => t.includes('React') || t.includes('Next'))) return '⚛️';
    if (tech.some(t => t.includes('Vue'))) return '🟢';
    if (tech.some(t => t.includes('JavaScript'))) return '🟨';
    if (tech.some(t => t.includes('TypeScript'))) return '🔷';
    if (tech.some(t => t.includes('Tailwind'))) return '🎨';
    return '💻';
  };

  return (
    <div className={`${className} bg-gradient-to-br ${getGradient(project.id)} flex items-center justify-center`}>
      <div className="text-center text-white">
        <div className="text-3xl mb-2">{getIcon(project.technologies)}</div>
        <div className="text-sm font-semibold opacity-90">{project.technologies[0]}</div>
      </div>
    </div>
  );
};
